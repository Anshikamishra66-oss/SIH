...
> *A Smart India Hackathon (SIH) National Agricultural Digital Platform*
...
- `PUT /api/queue/:token/arrived` — *(Officer)* Mark arriving farmer as checked-in at the mandi gate.
- `PUT /api/queue/:token/call` — *(Officer)* Call a specific token to a designated counter.
- `POST /api/queue/call-next` — *(Officer)* Automatically call the next waiting farmer in line.
- `PUT /api/queue/:token/complete` — *(Officer)* Complete the serving token.

### ⚖️ Produce Procurement (`/api/procurements`)
- `GET /api/procurements/:id` — Get weighing slip, quality grade, and payment record.
- `POST /api/procurements` — *(Officer)* Create procurement record upon farmer arrival.
- `PUT /api/procurements/:id/status` — *(Officer)* Submit final measured quintals, grade (A/B/C), and release payout.
